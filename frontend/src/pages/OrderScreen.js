...
export default function OrderScreen() {
  ...
  // Safely handle location.state which may be undefined
  const { peopleCount: passedPeopleCount, existingOrderId } = location.state || {};
  const peopleCount = passedPeopleCount || 1;
  ...
  const sortedCategories = categories.length > 0 
    ...
    : ['Set Meals']; // Fallback when menu is loading
