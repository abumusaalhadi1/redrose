...
DEFAULT_PRINTER_IP = "192.168.1.146"
...
async def get_printer_config():
    ...
    if not config:
        return {"printer_ip": DEFAULT_PRINTER_IP}
    return {"printer_ip": config.get("printer_ip", DEFAULT_PRINTER_IP)}

@api_router.post("/printer/config")
async def update_printer_config(config: PrinterConfig):
    await db.settings.update_one(
        {"type": "printer"},
        {"$set": {"printer_ip": config.printer_ip}, "$unset": {"printer_port": ""}},
        upsert=True
    )
    return {"message": "Printer configuration updated"}

async def get_printer_target():
    printer_config = await db.settings.find_one({"type": "printer"}, {"_id": 0})
    printer_ip = printer_config.get("printer_ip", DEFAULT_PRINTER_IP) if printer_config else DEFAULT_PRINTER_IP
    return printer_ip, DEFAULT_PRINTER_PORT
...
async def send_additional_to_kitchen(order_id: str, item_indices: List[int]):
    ...
    PRINTER_IP, PRINTER_PORT = await get_printer_target()
...
async def spa_fallback(request: Request, exc: StarletteHTTPException):
    if exc.status_code == 404:
        ...
        if FRONTEND_BUILD_DIR.exists():
            if "." not in Path(request.url.path).name:
